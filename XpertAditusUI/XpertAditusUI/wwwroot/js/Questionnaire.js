...
function setRemainingTime() {
    $.ajax({
        url: window.location.origin + '/Test/UpdateRemainingTime?candidateResultId=' + window.testInfo.model.candidateResultId,
        type: 'POST',
...
function getQuestionnaire(TrainingContentId) {
    ...
    $.ajax({
        url: window.location.origin + '/Test/GetActiveTestInfo/' + window.CourseId + '/' + TrainingContentId,
        type: 'GET',
        success: function (result) {
            hideSpinner();
            if (result && !result.error) {
                ...
                for (i = 0; i <= result.maxCount - 1; i++) {
                    var buttonId = i + 1;
                    QuestionnaireHTML += '<button class="btn btn-outline-primary" id="btn_' + buttonId + '" onclick= "getQuestion(' + buttonId + ', undefined , true)">' + buttonId + '</button>'
                }
                ...
                getListOfActiveTestQuestions(TrainingContentId);
                AddBorder();
            }
            else {
...
function getListOfActiveTestQuestions(TrainingContentId) {
    showSpinner();
    $.ajax({
        url: window.location.origin + '/Test/GetListOfActiveTestQuestions?candidateResultId=' + window.testInfo.model.candidateResultId + '&TrainingContentId=' + TrainingContentId,
        type: 'GET',
        success: function (result) {
            hideSpinner();
            if (result && !result.error && result.length > 0) {
                ...
                getQuestion(-1, TrainingContentId);
            }
        }
    });
}
...
function SetColorButton() {
    if ($('input[name=answer]:checked').val() != undefined) {
        ...
        SaveQuestionAnswer(window.activeQuestion.questionnaireResultId, window.activeQuestion.optionSelected);
    }
    else {
...
function getQuestion(questionOrder = -1, TrainingContentId, IsDriectQuestion) {
    if (TrainingContentId == undefined) {
    ...
    if (questionOrder != -1) { // Driect Question No Condition
        ...
        if (checkQuestionOrderPresent) {
            ...
            $.ajax({
                url: window.location.origin + "/Test/GetActiveTestQuestion?questionOrder=-1&candidateResultId=" + window.testInfo.model.candidateResultId + "&TrainingContentId=" + TrainingContentId + "&driectQuestionNo=" + questionOrder,
                type: 'GET',
                success: function (result) {
                    hideSpinner();
                    if (result && !result.error) {
                        ...
                        if (result.id == "101") {
                            getQuestion(window.currentQuestionNo, TrainingContentId);
                        }
                        else {
                            alert(result.message);
                        }
        ...
        if (checkQuestionOrderPresent) {
            ...
            showSpinner();
            $.ajax({
                url: window.location.origin + '/Test/GetActiveTestQuestion?questionOrder=' + questionOrder + "&candidateResultId=" + window.testInfo.model.candidateResultId + "&TrainingContentId=" + TrainingContentId,
                type: 'GET',
                success: function (result) {
                    hideSpinner();
                    if (result && !result.error) {
...
function prevQuestion() {
    ...
    getQuestion(window.currentQuestionNo);
    AddBorder();
}
...
function Submit() {
    ...
    document.querySelectorAll('[id^="btnTrainingContentName"]').forEach((e) => window.trainingContent += e.innerText + " , ");
    window.trainingContent = window.trainingContent.slice(0, window.trainingContent.length - 2);
    if (confirm("Are you sure all sections " + window.trainingContent +"are completed"
    +" and you want to finish the test, once submit the test, score card will be generate based on the test.")) {
        SubmitTest();
        //CandidateResult();
...
function SaveQuestionAnswer(questionaireResultId, selectedOption) {
    var selectedOption = selectedOption.toString();
    var rootPath = window.location.protocol + "//" + window.location.host + "/Test/UpdateQuestionaireResult?questionaireResultId=" + questionaireResultId + "&selectedOption=" + selectedOption;
    $.ajax({
        url: rootPath,
        type: 'POST',
        async: false,
...
function SubmitTest() {
    ...
    var rootPath = window.location.protocol + "//" + window.location.host + "/Test/CompleteTest?candidateResultId=" + window.testInfo.model.candidateResultId;
    $.ajax({
        url: rootPath,
        type: 'Get',
        async: false,
